...
func (i IneligibleRegistration) Print() {
	fmt.Printf("%q ineligible for reason: %s\n", i.NativeAddr, i.Reason.Error())
}
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for {
				...
				if err != nil {
					errs = append(errs, fmt.Errorf("failed to parse bech32 address for old account %q: %w", oldFetchAddrString, err))
					continue
				}
				...
				if err != nil {
					errs = append(errs, fmt.Errorf("failed to parse bech32 address for new account %q: %w", registration.NativeAddress, err))
					continue
				}
				...
				if oldAcctBalance == nil {
					errs = append(errs, fmt.Errorf("old account with address %q does not have a balance", oldFetchAddr.String()))
					continue
				}
				...
				if newAcctBalance == nil {
					errs = append(errs, fmt.Errorf("new account with address %q does not have a balance", newFetchAddr.String()))
					continue
				}
				...
				if err != nil {
					errs = append(errs, fmt.Errorf("unable to parse amount of tokens to migrate for row %q: %w", row, err))
					continue
				}
				...
				bankGenesis.Balances[oldBalanceIndex].Coins = sdk.NewCoins()

				// Add migrated coins to new account balance
				newAcctBalance = newAcctBalance.Add(migrateCoins...)
				bankGenesis.Balances[newBalanceIndex].Coins = newAcctBalance
			...
			if outputPath != "" {
				if err := ioutil.WriteFile(outputPath, sortedBz, 0644); err != nil {
					return errors.Wrap(err, fmt.Sprintf("unable to write modified genesis file to %q", outputPath))
				}
			} else {
	...
	cmd.Flags().StringVarP(&outputPath, "output", "o", "./genesis_out.json", "path to write migrated genesis JSON data")
